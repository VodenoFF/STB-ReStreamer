...
def savePortalData(name, allChannels, allGenre):
    genre = name + "_genre" + ".json"
    name += ".json"
    name_path = os.path.join(parent_folder, name)
    namegenre_path = os.path.join(parent_folder, genre)
    with open(name_path, "w") as f:
        json.dump(allChannels, f, indent=4)
        print(f"Channels '{name}' saved")
    with open(namegenre_path, "w") as f:
        json.dump(allGenre, f, indent=4)
        print(f"Genres '{genre}' saved")
        ...
        if portals[portal]["enabled"] == "true":
            ...
            for mac in macs:
                try:
                    ...
                    name_path = os.path.join(parent_folder, portalName + ".json")
                    genre_path = os.path.join(parent_folder, portalName +"_genre"+ ".json")
